"""Five-request Anthropic cache plans from the shared source-authored suites.

Preview descriptions contain no nonce or outbound body. A job creates exactly
two nonces once and freezes all five rendered requests; replay never randomizes.
"""
...
class CacheSuiteDescription:
    ...
    @property
    def request_cap(self): return 5
    @property
    def requests(self):
        return tuple(CacheRequest(row["case_id"], row["label"], row["kind"], row["endpoint_constraint"]["url"], copy.deepcopy(row["expectation"]), None)
                     for row in _suite(self.snapshot, self.suite_id)["case_definitions"])
...
class CachePlan(CacheSuiteDescription):
    ...
    def requests(self):
        ...
        expected = {"schema_version": 1, "suite_id": self.suite_id, "snapshot_digest": self.snapshot["snapshot_digest"],
            "nonces": value.get("nonces"), "requests": _render(suite, value.get("nonces")), "request_cap": 5,
...
def build_cache_plan(snapshot, *, suite_id, endpoint=ENDPOINT, runs=1, job_type="param_test",
    ...
    require(suite_id in SUITE_IDS and endpoint == ENDPOINT and type(runs) is int and runs == 1 and job_type == "param_test",
            "Cache requires one explicit five-request parameter job on the exact endpoint")
    ...
    if create_runtime_nonce:
        ...
        value = {"schema_version": 1, "suite_id": suite_id, "snapshot_digest": snapshot["snapshot_digest"],
            "nonces": nonces, "requests": _render(suite, nonces), "request_cap": 5, "report_retention": "P1M",
            "nonce_generation_time": "job_creation_only"}
...
def next_cache_request(plan, records):
    ...
    for row in observed["case_results"]:
        if not row["response_valid"] or row["kind"] == "count" and not row["pass"]:
            return None
    return plan.requests[len(records)] if len(records) < 5 else None
