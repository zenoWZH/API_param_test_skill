...
`${CLAUDE_SKILL_DIR}` = skill 根目录（Claude Code 自动替换为本 SKILL.md 所在目录）；`$DATA` = 数据目录（默认 `~/.config/llm-api-test/`）。

## 1. 登录密码

- 首次 `console.sh start` 自动生成 `admin` + 随机密码，存于 `$DATA/console_auth.json`（PBKDF2 哈希）。
- 查看并告诉用户：`bash ${CLAUDE_SKILL_DIR}/scripts/console.sh passwd`
- 用户改密码：`bash ${CLAUDE_SKILL_DIR}/scripts/console.sh passwd --set <新密码>`（立即生效，已有会话保持有效）
- 重置随机密码：`bash ${CLAUDE_SKILL_DIR}/scripts/console.sh passwd --reset`
- 密码也可用环境变量固定：`WEB_CONSOLE_USER` / `WEB_CONSOLE_PASSWORD`（此时 `passwd` 会提示去环境变量处改）。
- 认证是**可选**的：`LLM_API_TEST_DISABLE_AUTH=1 bash ${CLAUDE_SKILL_DIR}/scripts/console.sh start` 关闭（仅限可信网络）。

## 2. 公网方式一：Cloudflare 快速隧道（零配置，推荐先试用）

```bash
bash ${CLAUDE_SKILL_DIR}/scripts/console.sh tunnel
...
- 停止：`bash ${CLAUDE_SKILL_DIR}/scripts/console.sh tunnel-stop`。
...
3. 创建页会显示安装命令，其中 `--token` 后面那串就是隧道 token，复制发给 Claude Code。
4. 在隧道的 **Public Hostname** 页添加：子域名如 `llm-test`、域名选自己的域名、`Service` 类型 `HTTP`、URL 填 `127.0.0.1:8090`（本项也可由隧道侧忽略，以 dashboard 为准）。

**Claude Code 侧：**

```bash
bash ${CLAUDE_SKILL_DIR}/scripts/console.sh tunnel --token <用户给的 token>
